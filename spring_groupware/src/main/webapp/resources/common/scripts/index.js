...
window.onpopstate = function(event) {
	if (document.location.href.lastIndexOf("index.htm") + 9 == document.location.href.length) {
		onTopMenu("MENU00");
	} else if (event.state != null) {
		//앞으로가기 할 경우 해당페이지 앞으로의 기록이 모두 없어짐
		onTopMenu(event.state);
	}
};
...
function menuA(menu, mCode) {
    ...
    for(var i = 0; i < menu.subMenuCode.length; i++) {
        ...
        a.attr("onclick", "onTopMenu('"+subMenu.mCode+"', '"+subMenu.upCode+"');");
        if(subMenu.codeLevel==1 && !subMenu.upCode){  // 추후 수정 검토 ....2020.03.23 김형민
        	a.attr("onclick", "");
        } 
        ...
        if (subMenu.isCategory == 1) {        	
            a.addClass("dropdown-toggle");
        }
        
        var icon = $("<i class='menu-icon fa'></i>");
        if (subMenu.codeLevel != 2) {
        ...
        if (subMenu.subMenuCode!=null && subMenu.subMenuCode.length > 0 && subMenu.isCategory == 1) {
        ...
        if (subMenu.subMenuCode!=null && subMenu.subMenuCode.length > 0 && subMenu.isCategory == 1) {
            sub.append(menuA(subMenu,mCode));
        }
...
function loadSidebar(mCode, sCode) {
    $.ajax({ 
        ...
        error: function(xhr, exception) {
        	if(xhr.status==302){
        		alert("세션이 만료되었습니다. \n 다시 로그인 하세요.!");
        		location.href="/index.htm?mCode="+mCode;
        	}
        }        
    });
}
        ...
        success: function(str) {
            ...
            if (oldDate == null || oldDate == "") {                                        // 최초 데이터 이면, 
                ...
                var noteMessage = '<a href="#" style="font-family:맑은 고딕; font-size:10pt; font-weight:bold;" onclick="newNoteOpen(\''+data.noteid+'\')"> '
				                + '    <img style="position:relative; top:-1px;" src="/resources/common/images/icon-mail-temp.png" align="absmiddle"> '
...
function getPasswordPopup() {
	$.ajax({
	    url: '/commons/getCountNewPasswordNeeded.htm', dataType: 'json',
	    success: function(data, text, request) {
	    	var msg = "";
	    	if (data.msg != "") {
...
function getlastLoginTime() {
  $.ajax({
      url: '/commons/getLastLoginTime.htm',
      type: 'POST',
      dataType: 'html',
      success: function(data, text, request) {
...
function getChatData2() {
  ...
  $(".tabs-chart").html("");
  $.ajax({
      url: '/commons/getChatData2.htm?date='+yesterday,
      type: 'POST',
      dataType: 'json',
      success: function(data, text, request) {
          new Morris.Donut({
  ...
  $.ajax({
      url: '/commons/getChatData2.htm?date='+today,
      type: 'POST',
      dataType: 'json',
      success: function(data, text, request) {
          new Morris.Donut({
  ...
  $.ajax({
      url: '/commons/getChatData2.htm?date='+tomorrow,
      type: 'POST',
      dataType: 'json',
      success: function(data, text, request) {
          new Morris.Donut({
